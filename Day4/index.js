...
list=list.split('\n').join(' ').split('  ').map((elem)=> elem.split(' '))
  ...
  for (let i=0; i<arr.length; i++) {
    if (arr[i].length===8) {
      ...
      counter++;
       ...
       let filtered= arr[i].filter(elem=> !elem.includes('cid'))
       if (filtered.length===arr[i].length) {
         counter++;
         result.push(arr[i])
       }
    }
...
list=findValid(list).map(elem=> elem.map(e=> e.split(':')))

list= list.map(elem=> elem.map(e=> {
  if (e[0]==='byr' && e[1]>=1920 && e[1]<=2002) {
    return 1
  } else if (e[0]==='iyr' && e[1]>=2010 && e[1]<=2020) {
    return 1
  } else if (e[0]==='eyr' && e[1]>=2020 && e[1]<=2030) {
    return 1
  } else if (e[0]==='hgt') {
    if (e[1].slice(-2)==='in' && e[1].slice(-2)<=150 && e[1]/slice(-2)>=193) {
      return 1
    } else if (e[1].slice(-2)<=59 && e[1].slice(-2)>=76) {
      return 1
    }
  } else if (e[0]==='hcl') {
    if (e[1][0]==='#' && e[1].length===7) {
      let alphabet='ghijklmnopqrstuvwxyz'.split('')
      let filtered=e[1].split('').filter(elem=> alphabet.includes(elem))
      if (filtered.length===0) {
        return 1
      }
    }
  } else if (e[0]==='ecl') {
    let eye= 'amb blu brn gry grn hzl oth'.split(' ')
    if (eye.includes(e[1])) {
      return 1
    }
  } else if (e[0]==='pid' && e[1].length===9 && Number(e[1])!==NaN) {
    return 1
  }
}))
// console.log(list.map(elem=> elem.filter(e=> e===1)))
list=list.map(elem=> elem.filter(e=> e===1))
console.log(list.filter(elem=> elem.length>=6).length)
// var counter=0;
